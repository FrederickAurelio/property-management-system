...
**Rates:** `UnitType` is the template (kWh, m³, min kWh, add-ons, monthly maint/admin). Create copies that card onto the reservation as stay-level fallback. Each billed month then freezes its own full card (`ReservationUtilityPeriodScheme`). Add period copies **live** unit-type defaults into the new month only; first unsaved period on a stay still uses the create-time snapshot. Changing UnitType or a later month’s rules does not rewrite saved months.

**Storage:** `rentAmountIdr` + denorm utility sums on `Reservation`; detail rows in `ReservationUtilityReading` (elec/water), `ReservationMaintenanceCharge`, and `ReservationUtilityPeriodScheme`. Stay-level rate columns remain create-time fallback + last-period denorm (not the billing source after a utilities save). Domain writes `totalAmountIdr` only via `recomputeStayQuoteTotal` after rent or utilities changes, then `syncPaidFromMovements`.
