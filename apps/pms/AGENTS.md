...
- **Reports** (`/reports`): period summary (cash In mixed · Out = refunds + expenses · billed lens · occupancy · source mix · compare · CSV) via `GET /staff/reports/summary`; ADMIN/SUPER_ADMIN only
- **Expenses** (`/expenses`): property operating cash-out ledger (`GET/POST/PATCH/DELETE /staff/expenses`); ADMIN+; Reports summarizes, this page writes
