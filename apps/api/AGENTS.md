...
- **Unit occupancy (date picker):** `GET /staff/units/:id/occupancy?yearMonth=YYYY-MM` — occupying stays (busy end = `inventoryEndDate`) + calendar blocks for one month; FE caches months as the calendar pages
