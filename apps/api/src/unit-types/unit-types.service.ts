...
import { toPublicUnitType } from '../common/inventory/inventory-mapper.js';
...
export class UnitTypesService {
  ...
  async listByProperty(
    ...
    return {
      items: rows.map((row) => toPublicUnitType(row, row._count.units)),
      pageInfo: buildPageInfo(query.page, query.pageSize, total),
    };
  }

  async getById(id: string): Promise<PublicUnitType> {
    ...
    return toPublicUnitType(row, row._count.units);
  }
  ...
  async create(
    ...
    try {
      ...
      return toPublicUnitType(created, 0);
    } catch (error: unknown) {
  ...
  async update(id: string, dto: UpdateUnitTypeDto): Promise<PublicUnitType> {
    ...
    try {
      ...
      return toPublicUnitType(updated, updated._count.units);
    } catch (error: unknown) {
      this.rethrowCodeConflict(error);
      throw error;
    }
