...
export class PropertiesService {
  ...
  async list(query: ListPropertiesQueryDto): Promise<Paginated<StaffProperty>> {
  ...
  async getById(id: string): Promise<StaffProperty> {
    ...
    return toStaffProperty(row, {
      typeCount: row._count.unitTypes,
      unitCount: row._count.units,
    });
  }

  async create(dto: CreatePropertyDto): Promise<StaffProperty> {
    ...
    try {
      ...
      return toStaffProperty(created, { typeCount: 0, unitCount: 0 });
    } catch (error: unknown) {
  ...
  async update(id: string, dto: UpdatePropertyDto): Promise<StaffProperty> {
    ...
    try {
      ...
      return toStaffProperty(updated, {
