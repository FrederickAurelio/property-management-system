...
import { toPublicUnit } from '../common/inventory/inventory-mapper.js';
...
export class UnitsService {
  ...
  async listByProperty(
    ...
    return {
      items: rows.map(toPublicUnit),
      pageInfo: buildPageInfo(query.page, query.pageSize, total),
    };
  }

  async getById(id: string): Promise<PublicUnit> {
    const row = await this.prisma.unit.findUnique({ where: { id } });
    if (!row) {
      throw new NotFoundException('Unit not found');
    }
    return toPublicUnit(row);
  }

  async create(propertyId: string, dto: CreateUnitDto): Promise<PublicUnit> {
    await this.assertPropertyExists(propertyId);
  ...
  async update(id: string, dto: UpdateUnitDto): Promise<PublicUnit> {
